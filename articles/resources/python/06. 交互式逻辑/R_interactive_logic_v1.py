...
def main():
    # 1. 查找文件
    files = find_files(".csv") # 假设是 CSV
    
    # 2. 获取选中的路径列表
    chosen_list = choose_file(files, desc="数据文件")
    
    if chosen_list and make_sure():
        print(f"\n正在调用 R 进行绘图...")
        
        # 将路径列表转换为字符串，用空格分隔
        files_str = " ".join(chosen_list)
        
        # 调用 R 脚本，并将文件列表作为参数传递
        # 假设你的 R 脚本名为 plot_results.R
        try:
            subprocess.run(["Rscript", "plot_results.R", files_str], check=True)
            print("绘图任务完成！")
