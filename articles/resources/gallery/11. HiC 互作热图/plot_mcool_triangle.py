...
def main() -> None:
    ...
    try:
        ...
        fail(
            f"无法打开 {uri}\n{error}\n"
            f"请运行 `cooler ls {mcool_path}` 检查可用分辨率。"
        )
    ...
    if CHROMS is None:
        ...
        try:
            selected_chroms = parse_chrom_selection(
        ...
        missing = [
            chrom for chrom in CHROMS if chrom not in chromsizes.index
        ]
        if missing:
    ...
    if n_bins > MAX_BINS:
        ...
        suggested = int(
            np.ceil(
                RESOLUTION * n_bins / MAX_BINS / 10_000
            ) * 10_000
        )
        fail(
            f"矩阵包含 {n_bins:,} 个 bins，单个矩阵约需 "
    ...
    # 只保留主对角线以上的一半；坐标变换后主对角线成为水平底边。
    row_numbers = np.arange(n_bins)[:, None]
    column_numbers = np.arange(n_bins)[None, :]
    triangle_matrix = np.ma.masked_where(row_numbers > column_numbers, matrix)
    ...
    # (x, y) -> ((x + y) / 2, (x - y) / 2)，把半个正方形变为
    # 底边位于 y=0 的等腰三角形。
    triangle_transform = (
        Affine2D.from_values(0.5, 0.5, 0.5, -0.5, 0, 0)
        + axis.transData
    )
    image = axis.imshow(
        triangle_matrix,
    ...
    for boundary in ends[:-1]:
        ...
        axis.plot([boundary, boundary], [0, boundary], **line_style)
        axis.plot([boundary, n_bins], [boundary, boundary], **line_style)

    axis.set_xlim(0, n_bins)
    axis.set_ylim(0, n_bins / 2)
    ...
    axis.set_title(
        f"Triangular Hi-C contact map ({RESOLUTION:,} bp)",
        pad=12,
    )
    ...
    colorbar = figure.colorbar(
        image, ax=axis, fraction=0.035, pad=0.025, shrink=0.82
    )
    value_name = (
        "balanced contact" if BALANCED else "raw contact count"
    )
    colorbar.set_label(
        f"log1p({value_name}); upper "
        f"{CLIP_QUANTILE * 100:g}% clipped"
    )
