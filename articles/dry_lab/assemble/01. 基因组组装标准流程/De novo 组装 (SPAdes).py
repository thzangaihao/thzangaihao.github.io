...
def check_env(tools=None):
    if tools is None:
        tools = ['spades.py', 'quast.py']
    for tool in tools:
...
def get_r2_path(r1_path):
    """根据常见双端测序命名规则推导 R2 文件路径。"""
    return r1_path.replace('_R1.fq', '_R2.fq')\
                  .replace('_1.fq', '_2.fq')\
...
def choose_samples():
    ...
    # 原始数据和已经由 fastp 清洗的数据均允许作为输入，但只展示配对完整的任务。
    valid_samples = []
    for r1_path in sorted(set(raw_files)):
        r2_path = get_r2_path(r1_path)
        if os.path.isfile(r2_path):
            valid_samples.append(r1_path)
        else:
            print(f"⚠️ 跳过未配对的 R1 文件: {os.path.relpath(r1_path, base_dir)}")
            print(f"   未找到对应 R2: {os.path.relpath(r2_path, base_dir)}")
    
    if not valid_samples:
        print("⚠️ 提示：未在脚本所在目录及任何子目录中找到双端测序样本 (R1文件)！")
        return []

    print(f"\n📂 自动扫描到以下 {len(valid_samples)} 个双端配对任务:")
    for i, f in enumerate(valid_samples, 1):
        r2_path = get_r2_path(f)
        print(f"\n  [{i}] 配对任务")
        print(f"      R1: {os.path.relpath(f, base_dir)}")
        print(f"      R2: {os.path.relpath(r2_path, base_dir)}")
    ...
    while True:
        try:
            ...
            for r1_path in selected_paths:
                ...
                # 文件名含 clean 时视为已清洗数据，后续跳过 fastp。
                is_clean = 'clean' in os.path.basename(r1_path).lower()
                ...
                if is_clean:
                    sample_name = sample_name.rstrip('_-.').removesuffix('_clean').removesuffix('-clean')

                final_samples.append((sample_name, r1_path, r2_path, is_clean))
...
def run_pipeline(sample_name, r1, r2, is_clean, threads, memory, current_idx, total_samples, batch_out_dir):
    ...
    try:
        # 1. 原始数据先用 fastp 清洗；已清洗数据直接用于组装。
        if is_clean:
            print(f"\n--- [1/3] 检测到 clean 数据，跳过 Fastp ---")
...
if __name__ == "__main__":
    try:
        ...
        if samples_to_run:
            if any(not sample[3] for sample in samples_to_run):
                check_env(['fastp'])
            ...
            for i, (sample_name, r1, r2, is_clean) in enumerate(samples_to_run, 1):
                run_pipeline(sample_name, r1, r2, is_clean, threads, memory, i, total, batch_out_dir)
